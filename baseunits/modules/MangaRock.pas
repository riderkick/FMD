...
begin
  ...
  if MangaInfo.FHTTP.GET(ModuleApiUrl + '/query/web401/info?country=United%20States&oid=' + mangaId) then
  begin
    Result := NO_ERROR;
...
begin
  ...
  if DownloadThread.FHTTP.GET(ModuleApiUrl + '/query/web401/pages?country=United%20States&oid=' + chapterId) then
  begin
    Result := True;
...
begin
  ...
  if MangaInfo = nil then Exit(UNKNOWN_ERROR);
  MangaInfo.FHTTP.MimeType := 'application/json';
  if MangaInfo.FHTTP.POST(ModuleApiUrl + '/query/web401/mrs_filter?country=United%20States', DirRequest) then
  begin
    Result := NO_ERROR;
    mangaList.Clear;
    with TXQueryEngineHTML.Create(MangaInfo.FHTTP.Document) do
