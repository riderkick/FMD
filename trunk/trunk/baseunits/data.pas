...
uses
  synautil, HTMLParser,
  FastHTMLParser in 'E:\FreeSpace\FMD\trunk\baseunits\fasthtmlparser\fasthtmlparser.pas', LConvEncoding,
  HTMLUtil in 'E:\FreeSpace\FMD\trunk\baseunits\fasthtmlparser\HTMLUtil.pas',
...
var
  ...
  function   GetMangaGoDirectoryPage: Byte;
  var
    i: Cardinal;
  begin
    Result:= INFORMATION_NOT_FOUND;
  ...
  function   GetLEEDirectoryPage: Byte;
  var
    i: Cardinal;
  begin
    Result:= INFORMATION_NOT_FOUND;
...
begin
  ...
  if website = MANGAGO_NAME then
    Result:= GetMangaGoDirectoryPage
  else
...
var
  ...
  function   AnimeStoryGetNameAndLink: Byte;
  var
    tmp: Integer;
    i: Cardinal;
    s: String;
  ...
  function   LEEGetNameAndLink: Byte;
  var
    tmp: Integer;
    i: Cardinal;
    s: String;
  ...
  function   ScanMangaGetNameAndLink: Byte;
  var
    tmp: Integer;
    i: Cardinal;
    s: String;
  ...
  // get name and link of the manga from MangaPanda
  function   MangaGoGetNameAndLink: Byte;
  var
    tmp: Integer;
    i: Cardinal;
    s: String;
  begin
    Result:= INFORMATION_NOT_FOUND;
    if NOT GetPage(TObject(source), WebsiteRoots[MANGAGO_ID,1] + MANGAGO_BROWSER + IntToStr(StrToInt(URL)+1) + '/', 0) then
    begin
      Result:= NET_PROBLEM;
      source.Free;
      exit;
  ...
  begin
    ...
    // get name and link of the manga from Kivmanga
  function   KivmangaGetNameAndLink: Byte;
  var
    tmp: Integer;
    i: Cardinal;
    s: String;
  begin
    Result:= INFORMATION_NOT_FOUND;
    if NOT GetPage(TObject(source), WebsiteRoots[KIVMANGA_ID,1], 0) then
    begin
      Result:= NET_PROBLEM;
      source.Free;
      exit;
...
begin
 ...
 { if website = OURMANGA_NAME then
  ...
  if website = ANIMESTORY_NAME then
    Result:= AnimeStoryGetNameAndLink
  else
  if website = LEE_NAME then
    Result:= LEEGetNameAndLink
  else
  if website = SCANMANGA_NAME then
    Result:= ScanMangaGetNameAndLink
  else
...
begin
  ...
  Parser.Free;
  source.Free;
  mangaInfo.website:= ANIMESTORY_NAME;
  // using parser (cover link, summary, chapter name and link)
  if parse.Count=0 then exit;
  for i:= 0 to parse.Count-1 do
  begin
...
begin
  ...
  Parser.Free;
  source.Free;
  mangaInfo.website:= SCANMANGA_NAME;
  // using parser (cover link, summary, chapter name and link)
  if parse.Count=0 then exit;
  for i:= 0 to parse.Count-1 do
  begin
